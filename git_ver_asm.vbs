...
const VERSION_TABLE_LOCATION = "1FFE0"
...
Sub push_back_data(aryContent,strData)
	...
	For nChar = 1 To Len(strData)
		push_back aryLine, FormatDataByte(Mid(strData,nChar,1))
		If (nChar = Len(strData)) or ( (nChar mod DB_PER_LINE) = 0 ) Then
			push_back aryContent, VbTab & "DB" & vbTab & "H'" & join(aryLine,"', H'") & "'"
			aryLine=Array()
		End If		
	Next
